...
describe('TimeLibraryFactory 测试', () => {
  ...
  test('应该有默认的时间库类型', () => {
    expect(factory.getCurrentType()).toBe(TimeLibraryType.MOMENT);
  });

  test('应该能够切换时间库类型', () => {
    factory.setLibraryType(TimeLibraryType.MOMENT);
    expect(factory.getCurrentType()).toBe(TimeLibraryType.MOMENT);
  });

  test('应该返回正确的时间库实例', () => {
    factory.setLibraryType(TimeLibraryType.MOMENT);
    const momentInstance = factory.getInstance();
    expect(momentInstance).toBeInstanceOf(MomentTimeLibrary);
  });
  ...
  test('应该在切换类型时创建新实例', () => {
    ...
    // 应该是不同的实例（因为重新创建了）
    expect(instance1).toBeInstanceOf(MomentTimeLibrary);
    expect(instance2).toBeInstanceOf(MomentTimeLibrary);
  });
...
describe('TimeLibraryType 常量测试', () => {
  test('应该包含正确的时间库类型', () => {
    expect(TimeLibraryType.MOMENT).toBe('moment');
  });
